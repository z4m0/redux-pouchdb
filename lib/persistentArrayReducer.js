...
const initializePersistentArrayReducer = async (storeGetter, db, reducerName, saveArrayReducer) => {
  try {
    const store = await waitAvailability(storeGetter);

    // dispatch updates
    const res = await db.allDocs({ include_docs: true });
    await Promise.all(res.rows.map(row => updateArrayReducer(store, row.doc, reducerName)));
...
const persistentArrayReducer = (storeGetter, db, reducerName) => reducer => {
  ...
  initializePersistentArrayReducer(storeGetter, db, reducerName, saveArrayReducer);

  return (state, action) => {
    // console.log('reducer', action.type, state)
    if (action.type === UPDATE_ARRAY_REDUCER && action.reducerName === reducerName && action.doc) {
    ...
    const init = async () => {
      const success = await waitAvailability(() => isInitialized[reducerName] && isArrayUpToDate(reducerName));
